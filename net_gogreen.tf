...
resource "aws_security_group" "app_sg" {
  ...
  ingress {
    ...
    to_port     = 80
    protocol    = "tcp"
    security_groups = [aws_security_group.alb_sg.id] ### i need to check!!!
  }
...
resource "aws_lb" "app_lb" {
  ...
  subnets            = [aws_subnet.public_a.id, aws_subnet.public_b.id]  # Use at least two subnets
...
resource "aws_launch_configuration" "web_lc" {
  ...
  image_id      = "ami-0ebfd941bbafe70c6"  # Ensured my valid AMI ID
...
resource "aws_autoscaling_group" "web_asg" {
  ...
  max_size            = 10
  desired_capacity    = 4
  vpc_zone_identifier = [aws_subnet.public_a.id, aws_subnet.public_b.id]  # Use at least two subnets
...
resource "aws_cloudwatch_metric_alarm" "http_error_alarm" {
  ...
  metric_name          = "HTTPCode_ELB_4XX_Count"  # Corrected metric name
